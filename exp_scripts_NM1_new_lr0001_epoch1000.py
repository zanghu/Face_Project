...
#1.读取数据集
dsy_train = YiSunNM1(which_set='train', start=0, stop=10000, gcn_zca=True, y_float=True, one_zero=False)
dsy_test = YiSunNM1(which_set='test', start=0, stop=3466, gcn_zca=True, y_float=True, one_zero=False)
...
#一个带有非线性的全连接层
fc_layer_h5 = Tanh(dim=100, layer_name='h5_fc', irange=0.05)

#一个只有线性变化的输出层 
fc_layer_h6 = LinearOutput(dim=6, layer_name='h6_fc', irange=0.05); #当使用该层作为最后一层时，mlp应该可以使用get_default_cost()

#4.制造mlp
layers = [crl_layer_h1, crl_layer_h2, crl_layer_h3, crl_layer_h4, fc_layer_h5, fc_layer_h6]
print layers[-1].layer_name

mlp_model = MLP(batch_size=100, input_space=Conv2DSpace(shape=[31, 39], num_channels=3, axes=('b', 0, 1, 'c')), layers=layers)
...
alg = SGD(batch_size=100, learning_rate=0.0001, init_momentum=None, monitoring_dataset=monitoring_dataset, cost=cost,\
          termination_criterion=EpochCounter(max_epochs=1000))

extensions = [MonitorBasedSaveBest(channel_name='test_objective', save_path='concolutional_network_best_NM1.pkl')]#, \
